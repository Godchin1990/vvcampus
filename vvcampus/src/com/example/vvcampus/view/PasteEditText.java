...
public class PasteEditText extends EditText {
	...
	public boolean onTextContextMenuItem(int id) {
		if (id == android.R.id.paste) {
			...
			if (text.startsWith(DialogActivity.COPY_IMAGE)) {
				// intent.setDataAndType(Uri.fromFile(new
				// File("/sdcard/mn1.jpg")), "image/*");
				text = text.replace(DialogActivity.COPY_IMAGE, "");
				Intent intent = new Intent(context, AlertDialog.class);
				String str = "Send_the_following_pictures";
				intent.putExtra("title", str);
				intent.putExtra("forwardImage", text);
				intent.putExtra("cancel", true);
				((Activity) context).startActivityForResult(intent,
						DialogActivity.REQUEST_CODE_COPY_AND_PASTE);
				// clip.setText("");
			}
		}
	...
	protected void onTextChanged(CharSequence text, int start,
		...
		if (!TextUtils.isEmpty(text)
				&& text.toString().startsWith(DialogActivity.COPY_IMAGE)) {
			setText("");
		}
